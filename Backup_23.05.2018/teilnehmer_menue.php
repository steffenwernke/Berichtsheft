...
if($_SESSION['benutzer'] == "teilnehmer"){
	...
	$querystart = "SELECT AusbildungStartDatum FROM teilnehmer WHERE teilnehmer.Tname = '$Tname'";
	
	
	$stmt = $conn->prepare($querystart); 
	$stmt->execute();
	$stmt = $stmt->get_result();
	
	$row = $stmt->fetch_row();
	
	// Datum AusbildungStart
	$ausbildungStartDatum = strtotime($row[0]);
	$ausbildungStartDatum= $ausbildungStartDatum - 259200;
...
$query = "SELECT AusbildungsWocheEnde, EBerichtsnummer, EName, EAbteilung FROM eintraege, teilnehmer WHERE eintraege.TNr = teilnehmer.TNr AND teilnehmer.Tname = '$Tname' AND AusbildungsWocheEnde <= DATE(now()) ORDER BY AusbildungsWocheEnde DESC LIMIT 1";
	...
	// Datum Datenbank Einträge
	$datumDatenbank=strtotime($row[0]);
	
	if($datumDatenbank < $ausbildungStartDatum){
		$datumDatenbank = $ausbildungStartDatum;
	}
	...
	$datumDatenbank = date("Y-m-d",$datumDatenbank);
	
	$datumDatenbank1=date("Y-m-d",strtotime($datumDatenbank));
	...
	$query = "Insert into eintraege (EStatus, EName, EAbteilung, EBerichtsnummer, TextBetrieblich, TextTheorie, AusbildungsWocheBeginn, AusbildungsWocheEnde, DatumUnterschrift, TNr) VALUES(0,'$EName','$EAbteilung','$EBerichtsnummer',NULL,NULL,'$wochemontag','$wochefreitag','$wochefreitag','$teilnehmernummer')";
...
// Anzeige: abgegebene Einträge //alle Status = 1 	
	...
	if(isset($_SESSION['EBerichtsnummer_offeneEintraege']) && isset($_SESSION['ENr_offeneEintraege'])){
	...
	if(isset($_SESSION['EBerichtsnummer_abgegebeneEintraege']) && isset($_SESSION['ENr_abgegebeneEintraege'])){
	...
	if(isset($_SESSION['EBerichtsnummer_fertigeEintraege']) && isset($_SESSION['ENr_fertigeEintraege'])){
